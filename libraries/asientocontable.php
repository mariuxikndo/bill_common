...
class Asientocontable {
    ...
    /* obtener el asiento contable cuando conocemos unicamente alguno de los documentos de pago */
    public function get_by_doc_pago($tipo_pago, $doc_id_pago, $estado = 1) {
        
        /* Primero obtenemos el id del asiento contable que contiene ese pago */
        $ac = $this->ci->generic_model->get_data( 'bill_asiento_contable_det', array( 'tipo_pago'=>$tipo_pago,'doc_id_pago'=>$doc_id_pago ), 'asiento_contable_id', null, 1 );
        
        /* y ahora obtenemos el detalle de ese asiento contable*/
        
        $join_cluase = array(
               '0' => array('table'=>'bill_asiento_contable ac','condition'=>'ac.id = acd.asiento_contable_id'),
               '1' => array('table'=>'billing_contacuentasplan pl','condition'=>'acd.cuenta_cont_id = pl.cod'),
               '2' => array('table'=>'billing_empleado empl','condition'=>'ac.user_id = empl.id')
            );
        
        if($estado == ''){
            $where_data = array( 'acd.asiento_contable_id'=>$ac->asiento_contable_id );
        }else{
            $where_data = array( 'acd.asiento_contable_id'=>$ac->asiento_contable_id, 'ac.estado'=>$estado );
        }
        
        $ac_data = $this->ci->generic_model->get_join(
                    'bill_asiento_contable_det acd', 
                    $where_data, 
                    $join_cluase, 
                    'ac.id, ac.anio, ac.fecha ac_fecha, ac.hora ac_hora, acd.debito, acd.credito, acd.detalle, acd.cuenta_cont_id, acd.tipo_pago, acd.doc_id acd_doc_id,acd.doc_id_pago, pl.nombre cta_name, empl.nombres user_nombres, empl.apellidos user_apellidos'
                );
